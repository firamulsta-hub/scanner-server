print("스캐너 5.0 시작 - 실전 장마감 + 위험모드 + 다기기 확인용")
...
# 0. 저장 폴더 설정
# =========================================
# 아래 경로를 OneDrive / Dropbox / Google Drive 폴더로 바꾸면
# 집 PC에서 만든 결과를 노트북/모바일에서도 바로 볼 수 있습니다.
OUTPUT_DIR = str(OUTPUT_STABLE_DIR)
...
# 시총 상위 800개만 사용
stocks = stocks.head(800).reset_index(drop=True)
...
try:
    ...
    if len(kospi_index) >= 60:
        ...
        idx_open = kospi_index["Open"].iloc[-1]
...
for i in range(len(stocks)):
    ...
    try:
        ...
        # 기본 제외
        if traded_value < 10000000000:   # 100억 미만 제외
            continue
        ...
        cond3 = last_volume > vol20 * 1.2
        cond4 = last_close >= high_20 * 0.96
        cond5 = change_5d <= 12
        cond6 = last_close > last_open
        cond7 = last_close >= low_20 * 1.12
        cond8 = change_20d >= 5
        cond9 = last_close < high_60 * 1.02
        cond10 = last_high >= high_20 * 0.98
        ...
        # 위험모드일 때 조건 강화
        if risk_mode:
            if traded_value < 20000000000:   # 200억 미만 제외
                continue
            if change_5d > 8:                # 급등 종목 더 강하게 제외
                continue
            if score < 8:                    # 점수 낮으면 제외
                continue
...
if len(base_df) > 0:
    ...
    for i in range(len(base_df)):
        ...
        try:
            ...
            cond_inst_2 = inst_positive_days >= 2
            ...
            investor_score = sum([
                cond_inst_1, cond_inst_2, cond_for_1, cond_for_2, cond_lastday
            ])
            ...
            # 위험모드면 수급도 더 엄격
            if risk_mode:
                pass_condition = investor_score >= 4
            else:
                pass_condition = investor_score >= 3

            if pass_condition:
                if row["차트등급"] == "A" and investor_grade == "A":
                    final_grade = "A"
                else:
                    final_grade = "B"
...
if len(final_df) > 0:
    ...
    prev_files = sorted(glob.glob(os.path.join(OUTPUT_DIR, "scanner5_final_*.csv")))
...
# 7. 저장
# =========================================
all_file = os.path.join(OUTPUT_DIR, f"scanner5_all_{today_str}.csv")
final_file = os.path.join(OUTPUT_DIR, f"scanner5_final_{today_str}.csv")
a_file = os.path.join(OUTPUT_DIR, f"scanner5_A_{today_str}.csv")
summary_file = os.path.join(OUTPUT_DIR, f"scanner5_summary_{today_str}.txt")
...
print("노트북/모바일에서는 이 파일들을 열어서 확인하면 됩니다.")

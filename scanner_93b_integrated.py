...
def _extract_json_object(stdout: str, filename: str) -> dict[str, Any]:
    text = (stdout or "").strip()
    if not text:
        raise ValueError(f"empty stdout: {filename}")

    # 가장 안전한 순서:
    # 1) 전체 그대로 파싱
    # 2) 모든 '{' 위치부터 object 파싱 시도
    # 3) 모든 '[' 위치부터 배열 파싱 시도 (dict로 감싸기)
    candidates: list[int] = []
    ...
    try:
        ...
        tried.append(f"full:{type(e).__name__}")
    ...
    preview = text[:800]
    raise ValueError(
        f"json output not found: {filename}\n"
        f"tries={tried[:20]}\n"
        f"stdout={preview}"
    )
...
def _run_python_script(filename: str) -> dict[str, Any]:
    ...
    parsed = _extract_json_object(result.stdout or "", filename)

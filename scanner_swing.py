print("스캐너 5.1 시작 - 공격형 스윙 / 1500종목")
...
# 0. 저장 폴더 설정
# =========================================
OUTPUT_DIR = str(OUTPUT_SWING_DIR)
...
for i in range(len(stocks)):
    ...
    try:
        ...
        cond3 = last_volume > vol20 * 1.15
        cond4 = last_close >= high_20 * 0.94
        cond5 = change_5d <= 18
        cond6 = last_close > last_open
        cond7 = last_close >= low_20 * 1.08
        cond8 = change_20d >= 3
        cond9 = last_close < high_60 * 1.05
        cond10 = last_high >= high_20 * 0.97
        ...
        # 위험모드일 때만 약간 강화
        if risk_mode:
            if traded_value < 5000000000:   # 50억 미만 제외
                continue
            if change_5d > 12:
                continue
            if score < 7:
...
if len(base_df) > 0:
    ...
    for i in range(len(base_df)):
        ...
        try:
            ...
            if pass_condition:
                if row["차트등급"] == "A" and investor_grade in ["A", "B"]:
                    final_grade = "A"
                else:
                    final_grade = "B"
...
if len(final_df) > 0:
    ...
    prev_files = sorted(glob.glob(os.path.join(OUTPUT_DIR, "scanner51_final_*.csv")))
...
# 7. 저장
# =========================================
all_file = os.path.join(OUTPUT_DIR, f"scanner51_all_{today_str}.csv")
final_file = os.path.join(OUTPUT_DIR, f"scanner51_final_{today_str}.csv")
a_file = os.path.join(OUTPUT_DIR, f"scanner51_A_{today_str}.csv")
summary_file = os.path.join(OUTPUT_DIR, f"scanner51_summary_{today_str}.txt")
